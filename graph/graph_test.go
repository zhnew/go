...
func TestGraph_GetEdges(t *testing.T) {
	...
	if count := len(g.GetEdges()); count != 24 {
		t.Fatalf("wrong number of edges in graph_01 : %d", count)
	}
	if count := len(g.GetOutEdge(StringID("C"))); count != 2 {
		t.Fatalf("wrong number of out edges of node C in graph_01 : %d", count)
	}
	if count := len(g.GetInEdge(StringID("S"))); count != 3 {
		t.Fatalf("wrong number of In edges of node S in graph_01 : %d", count)
	}
